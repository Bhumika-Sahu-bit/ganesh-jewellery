...
export const updateProduct = async (req, res) => {
  try {
    ...
    // update images (safe)
    if((req.files && req.files.length > 0) || req.body.imageUrls) {
      ...
      if(req.body.imageUrls) {
        let urls = JSON.parse(req.body.imageUrls);

        urls.forEach((urlObj) => {
          const url = typeof urlObj === "string" ? urlObj : urlObj.url;
          if (url && updatedImages.length < 8) {
        updatedImages.push({ url });
      }
        });
      }

      product.images = updatedImages; // replace , not push 
    }
